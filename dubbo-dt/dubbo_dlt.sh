...
read -p "Enter Agent load balancing strategy / 输入Agent负载均衡策略: " AGENT_LB
AGENT_LB=${AGENT_LB:-ConsistentHash}
read -p "Enter Agent total test mode / 输入Agent的测试方法(FIXED_COUNT : 固定次数模式/DURATION: 持续时长模式): " AGENT_TEST_MODE
AGENT_TEST_MODE=${AGENT_TEST_MODE:-FIXED_COUNT}
read -p "Enter Agent test duration in seconds / 输入Agent测试持续时间(秒): " AGENT_DURATION
AGENT_DURATION=${AGENT_DURATION:-100}

read -p "Enter Agent total test requests / 输入Agent测试请求总数: " AGENT_REQUEST_CNT
